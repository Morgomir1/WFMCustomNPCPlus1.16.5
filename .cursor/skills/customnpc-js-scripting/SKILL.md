...
> **Связанные skills:** `nashorn-customnpc-scripting` — Nashorn ES5.1, `Java.type`, bootstrap `NpcAPI`, стиль кода. `customnpc-java-abilities` — создание абилок в Java (`CnpcAbility`, `AbilityRegistry`). `customnpc-attack-zones` — Telegraph / Ability Zone. [script-java-helpers.md](script-java-helpers.md) — утилиты `ScriptDataUtil`, vampire helpers. Этот skill — события, API и **архитектура способностей**.
