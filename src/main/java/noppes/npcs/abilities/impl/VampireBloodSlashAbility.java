...
/**
 * Vampire sword sweep: truncated cone like {@link WhFlamingStrikeAbility}, blood VFX, no fire.
 */
public final class VampireBloodSlashAbility implements CnpcAbility {
    public static final String ID = "vampire_blood_slash";
    ...
    public Map<String, Object> defaultParams() {
        return AbilityDefaults.vampireBloodSlash();
    }
    ...
    public boolean onStart(final ActiveAbility active, final AbilityContext ctx) {
        ...
        ctx.world.playSoundAt(ctx.npc.getPos(), "minecraft:item.trident.throw", 0.75F, 0.7F);
        return true;
    }
    ...
    private TickResult tickCharge(final ActiveAbility active, final AbilityContext ctx) {
        ...
        if (active.ticksLeft % 3 == 0) {
            AbilityVfx.spawnBloodCharge(ctx.world, ctx.npc.getX(), ctx.npc.getY() + 1.0, ctx.npc.getZ());
        }
        active.ticksLeft--;
    ...
    private TickResult doStrike(final ActiveAbility active, final AbilityContext ctx) {
        ...
        AbilityVfx.spawnBloodSlashSweep(
                ctx.world,
                active.sx, active.sy, active.sz,
                active.ex, active.ey, active.ez,
                nearHalfWidth);
        AbilityVfx.spawnBloodBurst(ctx.world, active.ex, active.ey + 0.3, active.ez, nearHalfWidth);
        ctx.world.playSoundAt(
                NpcAPI.Instance().getIPos(active.ex, active.ey, active.ez),
                "minecraft:entity.player.attack.sweep",
                1.05F,
                0.75F);
        ctx.world.playSoundAt(ctx.npc.getPos(), "minecraft:entity.player.attack.crit", 0.85F, 0.8F);
