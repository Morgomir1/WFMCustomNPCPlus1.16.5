...
/**
 * Огненный удар: усечённый конус (у босса уже широкое основание) → взмах + поджог.
 */
public final class WhFlamingStrikeAbility implements CnpcAbility {
    public static final String ID = "wh_flaming_strike";
    ...
    public Map<String, Object> defaultParams() {
        return AbilityDefaults.whFlamingStrike();
    }
    ...
    public boolean onStart(final ActiveAbility active, final AbilityContext ctx) {
        ...
        // Вершина конуса позади босса → у ног уже широкое основание
        ...
        ctx.world.playSoundAt(ctx.npc.getPos(), "minecraft:item.firecharge.use", 0.85F, 1.1F);
        return true;
    }
    ...
    private TickResult doStrike(final ActiveAbility active, final AbilityContext ctx) {
        ...
        final int fireSeconds = ctx.params.getInt(AbilityParamKeys.FIRE_SECONDS, 4);
        ...
        AbilityCombatHelper.damageInTruncatedCone(
                ...
                minDist, maxDist,
                damage, knockback, knockbackY, fireSeconds);

        AbilityVfx.spawnFlamingStrikeSweep(
                ctx.world,
                active.sx, active.sy, active.sz,
                active.ex, active.ey, active.ez,
                nearHalfWidth);
        AbilityVfx.spawnFireRing(ctx.world, active.sx, active.sy + 0.2, active.sz, nearHalfWidth);
        AbilityVfx.spawnStartBurst(ctx.world, active.ex, active.ey, active.ez, false);
        ctx.world.playSoundAt(
                NpcAPI.Instance().getIPos(active.ex, active.ey, active.ez),
                "minecraft:entity.player.attack.sweep",
                1.0F,
                0.85F);
        ctx.world.playSoundAt(ctx.npc.getPos(), "minecraft:item.flintandsteel.use", 0.9F, 1.2F);
