...
public class ItemStackWrapper implements IItemStack, ICapabilitySerializable<CompoundNBT>
{
    ...
    public void damageItem(final int damage, final IMob living) {
        if (living != null) {
            this.item.hurtAndBreak(damage, (LivingEntity) living.getMCEntity(), e -> e.broadcastBreakEvent(EquipmentSlotType.MAINHAND));
        }
    ...
    public double getAttackDamage() {
        final Multimap<Attribute, AttributeModifier> map = (Multimap<Attribute, AttributeModifier>)this.item.getAttributeModifiers(EquipmentSlotType.MAINHAND);
        double damage = 0.0;
        for (final Map.Entry<Attribute, AttributeModifier> entry : map.entries()) {
