...
public class GuiNpcDisplay extends GuiNPCInterface2 implements ITextfieldListener, IGuiData {
   ...
   public void init() {
      ...
      y += 23;
      this.addLabel(new GuiLabel(17, "display.hitboxWidth", this.guiLeft + 5, y + 5));
      this.addTextField(new GuiTextFieldNop(17, this, this.guiLeft + 80, y, 50, 20, GuiTextFieldNop.formatFloat(this.display.getHitboxWidth())));
      this.getTextField(17).setFloatsOnly();
      this.getTextField(17).setMinMaxDefault(0.05f, 20.0f, 0.6f);
      this.addLabel(new GuiLabel(18, "display.hitboxHeight", this.guiLeft + 145, y + 5));
      this.addTextField(new GuiTextFieldNop(18, this, this.guiLeft + 220, y, 50, 20, GuiTextFieldNop.formatFloat(this.display.getHitboxHeight())));
      this.getTextField(18).setFloatsOnly();
      this.getTextField(18).setMinMaxDefault(0.05f, 20.0f, 1.8f);
      this.addLabel(new GuiLabel(19, "(0.05-20)", this.guiLeft + 275, y + 5));
      y += 23;
      this.addLabel(new GuiLabel(4, "display.texture", this.guiLeft + 5, y + 5));
      this.addTextField(
         new GuiTextFieldNop(3, this, this.guiLeft + 80, y, 200, 20, this.display.skinType == 0 ? this.display.getSkinTexture() : this.display.getSkinUrl())
      );
      this.addButton(new GuiButtonNop(this, 3, this.guiLeft + 325, y, 38, 20, "mco.template.button.select"));
      this.addButton(
         new GuiButtonNop(this, 2, this.guiLeft + 283, y, 40, 20, new String[]{"display.texture", "display.player", "display.url"}, this.display.skinType)
      );
      this.getButton(3).setEnabled(this.display.skinType == 0);
      if (this.display.skinType == 1 && !this.display.getSkinPlayer().isEmpty()) {
         this.getTextField(3).setValue(this.display.getSkinPlayer());
      }
      ...
      this.addButton(
         new GuiButtonBiDirectional(
            this, 13, this.guiLeft + 230, y, 100, 20, this.display.getHitboxState(), new String[]{"stats.normal", "gui.none", "hair.solid"}
         )
      );
      y += 23;
      ...
      this.addButton(
         new GuiButtonNop(
            this, 10, this.guiLeft + 60, y, 110, 20, new String[]{"display.hide", "display.show", "display.showAttacking"}, this.display.getBossbar()
         )
      );
   ...
   public void unFocused(GuiTextFieldNop textfield) {
      if (textfield.id == 0) {
         if (!textfield.isEmpty()) {
            this.display.setName(textfield.getValue());
         } else {
            textfield.setValue(this.display.getName());
         }
      } else if (textfield.id == 2) {
         this.display.setSize(textfield.getInteger());
      } else if (textfield.id == 17) {
         this.display.setHitboxWidth(textfield.getFloat());
      } else if (textfield.id == 18) {
         this.display.setHitboxHeight(textfield.getFloat());
      } else if (textfield.id == 3) {
         if (this.display.skinType == 2) {
            this.display.setSkinUrl(textfield.getValue());
         } else if (this.display.skinType == 1) {
            this.display.setSkinPlayer(textfield.getValue());
         } else {
            this.display.setSkinTexture(textfield.getValue());
         }
         ...
         this.display.setCapeTexture(textfield.getValue());
      } else if (textfield.id == 9) {
         this.display.setOverlayTexture(textfield.getValue());
      } else if (textfield.id == 11) {
         this.display.setTitle(textfield.getValue());
      }
   }
